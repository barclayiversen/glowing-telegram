...
void APIENTRY hkEndScene(LPDIRECT3DDEVICE9 o_pDevice)
{
	...
	{
		DrawText("ShowMenu (INS)", menuOffX, menuOffY, D3DCOLOR_ARGB(255, 255, 255, 255));
	}
	else {
	...
	for (int i = 1; i < 32; i++)
	{
		...
		if (hack->localEnt->iTeamNum != 0x2 && hack->localEnt->iTeamNum != 0x3)
		{
			hack->GetLocalEnt();
		}
		...
		if (hack->WorldToScreen(curEnt->vecOrigin, entPos2D))
		{
			...
			if (hack->WorldToScreen(entHead3D, entHead2D))
			{
				...
				{
					DrawEspBox3D(entHead3D, curEnt->vecOrigin, curEnt->angEyeAnglesY, 25, 1, espColor);
				}
				...
				if (hack->settings.headlineEsp)
				{
					...
					{
						DrawLine(head2D, endPoint2D, 2, espColor);
					}
				}
